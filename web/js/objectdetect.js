...
$(document).ready(function () {
    ...
    $("#submit").bind("click",function () {
        ...
        let config = {
            url:"imagecontrol?action=objectdetect",
            ...
            success:function (resp) {
                console.log(resp)
                // 动态标签更新
                $("#classes").text(resp.result[0].root);
                $("#result").text(resp.result[0].keyword);
                $("#analysis").text(resp.result[0].baike_info.description);
            },
            error:function (resp) {
